...
int init_frames(struct usbg_f_uvc *uvc, int j)
{
	...
	ret = snprintf(fpath, sizeof(fpath), "%s/%s/" UVC_PATH_STREAMING "/%s/",
			       ...
			       union usbg_f_uvc_frame_attr_val *val)
{
	...
	ret = snprintf(fpath, sizeof(fpath), "%s/%s/" UVC_PATH_STREAMING "/%s/frame.%d/",
...
int usbg_f_uvc_get_nframes(usbg_f_uvc *uvcf, const bool *frames)
{
	int nframes = 0;
	int i;

	for (frames, i = 0; i < MAX_FRAMES; ++i)
			 ...
			 int frame_id, struct usbg_f_uvc_frame_attrs *fattrs)
{
	...
	ret = snprintf(frame_path, sizeof(frame_path), "%s/%s/" UVC_PATH_STREAMING "/%s/frame.%d/",
		       uvcf->func.path, uvcf->func.name, format, frame_id);
	if (ret >= sizeof(frame_path))
		return USBG_ERROR_PATH_TOO_LONG;
			     ...
			     bool *frames, config_setting_t *root)
{
	...
	nframes = usbg_f_uvc_get_nframes(uvcf, frames);
	for (i = 0; i < nframes; ++i) {
